...
import { backendPostAddNewRedemption, backendPatchUpdateRedemption, backendGetRedemptionInfo, backendGetGiftInfo } from '../../helpers/backend_helper'
...
export default function OrderSave() {
    ...
    //const [requestData, setRequestData] = useState<OrdersCreateInterface>(initialOrders)
    const { id } = router.query
    const [productData, setProductData] = useState([])
    const [giftPoint, setGiftPoint] = useState(1)
    const fetchRedemptionDetail = async () => {
        await backendGetOrderInfo(id).then((res) => {
            if (!res.isError) {
    ...
    const handleFormSubmit = async () => {
        try {
          ...
          //  ['_id', 'active','giftid','coupon','invoiceNo' ,'quantity','redeemedpoints','createdAt', 'giftName','address','giftType','firmName','status'].forEach(e => delete iData[e]);
            var actionSubmit = await (formik.values.parentid) ?  backendPatchUpdateOrder(formik.values.parentid, iData) : backendPostAddNewOrder(iData)
    ...
    return (
        <Layout>
            <BreadcrumbComponent firstItem={{ href: '/dashboard', label: 'dashboard' }} secondItem={{ href: '/orders', label: 'OrderList' }} itemlabel='Orders List' />
            <Row>
                <Col xl={12} md={12}>
                    <Row className="align-items-center mb-4">
                        <Col md={6}>
                            <h3 className="card-body mb-3">{ (id) ? 'Edit' : 'Add' }Order</h3>
                        </Col>
